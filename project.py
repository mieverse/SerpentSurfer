...
def drawDisplay():
    ...
    drawText(10, 10, "W/S=move  A/D=turn  SPACE=knockback  ESC=menu")
...
def keyboardListener(key, x, y):
    ...
    if current_screen == "game" and not game_over and not game_won:
        ...
        if key == b's':
            nx = snake_pos[0] + (cur_speed * 0.5) * math.sin(math.radians(snake_angle))
            ny = snake_pos[1] - (cur_speed * 0.5) * math.cos(math.radians(snake_angle))
            moved = False
            if abs(nx) < effective_boundary - 20 and not hitsCube(nx, snake_pos[1]):
                snake_pos[0] = nx
                moved = True
            if abs(ny) < effective_boundary - 20 and not hitsCube(snake_pos[0], ny):
                snake_pos[1] = ny
                moved = True
            if moved:
                recordPos()
